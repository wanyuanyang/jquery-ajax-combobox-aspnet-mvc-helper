...
(function ($) {
    $.ajaxComboBox = function (area_pack, source, options, msg) {
        ...
        // This facilitates easy access on input and hidden tags, and resetting of page -- Buen
        this.box = null;
        ...
        if (options.package) {
            var $add_btn = $('<img />')
				...
				.click(function () {
				    addPack(this, area_pack); // this, added by Buen
				})
				.appendTo($add_area);
        }
        ...
        //ComboBoxを追加
        //************************************************************
        function addPack(c, btn) { // c, added by Buen
            num++;
            ...
            // individual($box); // removed by Buen
            c.box = new individual($box); // added by Buen
        ...
        function individual(area_combobox) {
            ...
            this.input = $input; // added by Buen
            ...
            // replaced+ by Buen
            var $result_area = $('<div style="position: fixed"></div>')
                .addClass(options.re_area_class);
            ...
            var $hidden = $('<input type="hidden" />')
				.attr({
				    ...
				    */
				    ...
				    // added by Buen...
				    'name': options.the_field_name,
				    'id': options.the_field_id
				    // ...added by Buen
				})
				.val('');


            this.hidden = $hidden; // added by Buen

            // $hidden. attr({ 'x': 'y' }); // sample adding of attribute                        
            $hidden.attr(options.other_attr); // added by Buen
            ...
            // Added by Buen...
            if (options.textbox_width != undefined) {
                $(area_combobox).width(options.textbox_width);
                // $input.css('width', options.textbox_width);
            }
            // ...Added by Buen
            ...
            function suggest() {
                ...
                if (type_suggest && q_word.length < options.minchars) {
                    ...
                    $.globalEval('var xxx = ' + options.cascaded_word + ';'); // added by Buen

                    //ここでAjax通信を行っている
                    // $xhr = $.getJSON( // replaced- by Buen
                    $xhr = $.post( // replaced+ by Buen. use Post, it's more secure
						options.source,
						{
						    'cascaded_word': xxx, // added by Buen
						    'q_word': q_word,
            ...
            function displayItems(arr_candidate, arr_attached, arr_primary_key) {
                ...
                var theLeft = $(area_pack).offset().left - $(document).scrollLeft();
    ...
    $.fn.ajaxComboBox = function (source, options) {
        ...
        // Added by Buen...
        if (options.can_render_callback != undefined) {
            var can_render = eval(options.can_render_callback)();
            // alert(" Can " + can_render);
        ...
        options = $.extend({
            ...
            // replaced by Buen.  from: acbox/img/
            img_dir: '/Scripts/Ienablemuch.Scripts/Ienablemuch.JqueryAjaxComboBox/img/',             //ボタン画像へのパス
            ...
            the_field_name: $(this).attr('id').replace(/_/g, '.'), // added by Buen
            the_field_id: $(this).attr('id'), // added by Buen
        ...
        var ajc = null; //added by Buen

        this.each(function () {
            // new $.ajaxComboBox(this, source, options, msg); // removed by Buen
            ajc = new $.ajaxComboBox(this, source, options, msg); // added by Buen
        });
